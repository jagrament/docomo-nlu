  ...
  describe "#configs" do
    it "Get configs" do
      VCR.use_cassette("/V26/config/index") do
        configs = DocomoNlu::Management::V26::Config.all(params: { project_id: 1, bot_id: "test_bot" })
        expect(configs).not_to be nil

        configs = DocomoNlu::Management::V26::Config.find(:all, params: { project_id: 1, bot_id: "test_bot" })
        expect(configs).not_to be nil

        configs = DocomoNlu::Management::V26::Config.where(project_id: 1, bot_id: "test_bot")
        expect(configs).not_to be nil
      end
    end
    ...
    it "Update configs" do
      VCR.use_cassette("/V26/config/show") do
        config = DocomoNlu::Management::V26::Config.find("", params: { project_id: 1, bot_id: "test_bot" })
