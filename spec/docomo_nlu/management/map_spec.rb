    ...
    context "Download zip" do
      it "Use all" do
        VCR.use_cassette("/management/map/index_all") do
          map = described_class.all(params: { project_id: project_id, bot_id: bot_id })
          expect(map.file.size).not_to be 0
        end
      end
      it "Use find" do
        VCR.use_cassette("/management/map/index_find") do
          map = described_class.find(nil, params: { project_id: project_id, bot_id: bot_id })
          expect(map.file.size).not_to be 0
        end
      end
      it "Use where" do
        VCR.use_cassette("/management/map/index_where") do
          map = described_class.where(project_id: project_id, bot_id: bot_id)
          expect(map.file.size).not_to be 0
        end
      end
    end
    ...
    context "Upload map" do
      it "Use class method create()" do
        VCR.use_cassette("/management/map/create") do
          ...
          res = described_class.create(File.new(File.join("spec", "fixtures", "management", "test.map")), attributes)
    ...
    context "Download map" do
      it "Use find" do
        VCR.use_cassette("/management/map/show_find") do
          map = described_class.find("test", params: { project_id: project_id, bot_id: bot_id })
          expect(map.file.size).not_to be 0
        end
      end
      it "Use where" do
        VCR.use_cassette("/management/map/show_where") do
          map = described_class.where(category: "test", project_id: project_id, bot_id: bot_id)
          expect(map.file.size).not_to be 0
        end
      end
    end
