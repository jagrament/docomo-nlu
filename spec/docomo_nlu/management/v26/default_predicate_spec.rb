  ...
  describe "#defaultPredicates" do
    ...
    it "Not found" do
      VCR.use_cassette("/V26/default_predicate/index_not_found") do
        default_predicates = DocomoNlu::Management::V26::DefaultPredicate.all(params: { project_id: project_id, bot_id: bot_id })
        expect(default_predicates).to eq []

        default_predicates = DocomoNlu::Management::V26::DefaultPredicate.find(:all, params: { project_id: project_id, bot_id: bot_id })
        expect(default_predicates).to eq []

        default_predicates = DocomoNlu::Management::V26::DefaultPredicate.where(project_id: project_id, bot_id: bot_id)
        expect(default_predicates).to eq []
      end
    end
    ...
    it "Update defaultPredicates" do
      VCR.use_cassette("/V26/default_predicate/index") do
        default_predicate = DocomoNlu::Management::V26::DefaultPredicate.all(params: { project_id: project_id, bot_id: bot_id }).first
